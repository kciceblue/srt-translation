...
### 两轮翻译
1. **第一轮 — 分块翻译**：将文本按小块（`--chunk-size`，默认 10 行）逐块翻译，小块翻译避免模型陷入循环。
2. **第二轮 — 上下文润色**：将所有源文本 + 初稿翻译一起发送给模型进行全文审校，修正名称不一致、上下文断裂和误翻。
3. **修正语音识别错误**：两轮翻译均指示模型修正 Whisper 的转录错误。
4. **编号 I/O**：使用 `[N] 文本` 格式精确追踪每一行。
