...
### 持久化词汇表
词汇表文件（默认 `vocab.txt`）在启动时自动加载，每次翻译结束后将新学到的术语合并写回。文件使用 `源词 → 译词` 格式，可在两次运行之间自由编辑。通过 `--vocab` 更改路径，设为 `--vocab ''` 禁用。词汇表同时用于翻译术语表和校对提示，确保命名一致。

### 校对步骤
启用 `--proofread` 后，同一系列的所有文件翻译完成后，会对每个文件进行第二轮校对。校对时将原文+译文配对发送给 LLM，并附带完整词汇表（`--vocab` 文件 + 系列翻译中积累的术语表）。校对器会修正 `??` 标记的行、不一致的名称和误译。如果校对失败，保留原始翻译。
